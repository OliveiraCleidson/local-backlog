...
1. **Ferramenta real:** CLI que o autor vai usar em todos os seus repositórios pessoais para gerenciar backlog, colar entre máquinas via `cargo install` e consumir com agentes de IA.
...
3. **Código gerado/assistido por IA passa por revisão humana explicativa.** O objetivo não é apenas funcionar, é o autor entender cada bloco não-trivial. Code review (humano ou adversarial via Codex) deve explicar idiomas quando opacos, não apenas aprovar.

Esta decisão é foundational — todos os demais ADRs (0001–NNNN) assumem Rust como pressuposto.

## Consequências

**Positivas:**
- Projeto pequeno vira campo de treino controlado: `tasks` atômica + satélites cobre `rusqlite`, migrations, triggers, serde, error handling tipado, CLI parsing — os idiomas centrais do ecossistema.
